MainMenuDriver = {
            ...
            el.addEventListener("click", async function (e) {
                ...
                switch (command) {
                    ...
                    case "comment":
                            ...
                            MainMenuDriver._escKeyHandler = function (evt) {
                                try {
                            ...
                            document.addEventListener('keydown', MainMenuDriver._escKeyHandler);
                ...
                // MainMenuDriver.hideMenu();
                                try { document.removeEventListener('keydown', MainMenuDriver._escKeyHandler); } catch (e) { }
                                MainMenuDriver._escKeyHandler = null;
    ...
    showMenu: function () {
        ...
        dialog.addEventListener('close', function () {
            ...
            try { document.removeEventListener('click', MainMenuDriver._outsideClickHandler); } catch (e) { }
            MainMenuDriver._outsideClickHandler = null;
            MainMenuDriver.menuVisible = false;
            MainMenuDriver.showUserSelect(false);
        });
        document.body.appendChild(dialog);
        MainMenuDriver.menuDialog = dialog;
        // If the user clicks the dialog backdrop (evt.target === dialog), close it.
        dialog.addEventListener('click', function (evt) {
            try {
                if (MainMenuDriver._ignoreNextClick) {
                    MainMenuDriver._ignoreNextClick = false;
                    return;
                }
        ...
        // Close the menu when clicking outside the dialog.
        // Set a short-lived flag to ignore the immediate release/click
        // that completes the press gesture which opened the menu.
        MainMenuDriver._ignoreNextClick = true;
        setTimeout(function () { MainMenuDriver._ignoreNextClick = false; }, 350);
        // Install a capturing click interceptor to suppress the synthetic
        // click generated by releasing the press. This runs in the capture
        // phase so it fires before bubble-phase handlers and can stop the
        // event reaching the outside-click handler.
        MainMenuDriver._suppressCapturingClick = function (evt) {
            try {
                if (MainMenuDriver._ignoreNextClick) {
            ...
            // remove the capturing listener after first use
            try { document.removeEventListener('click', MainMenuDriver._suppressCapturingClick, true); } catch (e) { }
            MainMenuDriver._suppressCapturingClick = null;
        };
        document.addEventListener('click', MainMenuDriver._suppressCapturingClick, true);
        // Attach the listener on the next tick so the click that opened
        // the menu doesn't immediately trigger it.
        MainMenuDriver._outsideClickHandler = function (evt) {
            try {
                // If we're currently ignoring the first click after open, skip it.
                if (MainMenuDriver._ignoreNextClick) {
                    MainMenuDriver._ignoreNextClick = false;
                    return;
                }
                if (evt.target === dialog || !dialog.contains(evt.target)) {
                    dialog.close();
                }
            } catch (e) { /* ignore */ }
        };
        setTimeout(function () { document.addEventListener('click', MainMenuDriver._outsideClickHandler); }, 0);
        dialog.showModal();
